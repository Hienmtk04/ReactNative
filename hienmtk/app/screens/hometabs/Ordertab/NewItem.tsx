...
const NewItem = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await GET_ALL("products?pageNumber=0&pageSize=50&sortBy=productId&sortOrder=desc");
        if (response && Array.isArray(response.content)) {
          // Filter products with discount = 0
          const filteredProducts = response.content.filter(product => product.discount === 0);
